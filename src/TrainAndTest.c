...
int   predictLabel(double *sample, int numFeatures)
{
     ...
     */
    for (int i=0;i < rulesInCandidate && thePrediction==NO_PREDICTION; i++)
    {
        thisRule.variableAffected = workingCandidate.variableValues[4*i];
        thisRule.comparison = workingCandidate.variableValues[4*i+1];
        thisRule.threshold = workingCandidate.variableValues[4*i+2];
        thisRule.prediction = workingCandidate.variableValues[4*i+3];
    ...
    if(modelTrained==true && thePrediction == NO_PREDICTION)
    {
        ...
        int index = rand()%(numClasses);
        thePrediction = validLabels[index];
        // thePrediction = 1;
    }
...
void GreedyConstructiveSearch(void)
{
  ...
  // initialise the variables used for search e.g. workingCandidate, openList and closedList
    ...
    while (atGoal==false)
    {
        ...
        { 
            for ( operator = lessThan; operator <= greaterThan; operator++)
            {
        ...
        candidateSolution * candidate = &tmp;
        candidateList *listToChange = &closedList;
        ...
        int i, j;
        candidateSolution tempCandidate;
        for (i = 0; i < openList.size; ++i) 
        {
            for (j = i + 1; j < openList.size; ++j) 
            {
                if (openList.listEntries[i].score < openList.listEntries[j].score) 
                {
                    tempCandidate = openList.listEntries[i];
                    openList.listEntries[i] = openList.listEntries[j];
                    openList.listEntries[j] = tempCandidate;
                }
            }
        }
        ...
        {
            RemoveSolutionFromOpenList(currentSolution);
        }
        openList.size = 0;
